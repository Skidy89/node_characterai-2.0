...
export class Candidate extends Specable {
  ...
  public get wasFlagged() {
    return this.safety_truncated;
  }
  public set wasFlagged(value) {
    this.safety_truncated = value;
  }

  // annotation
  async createAnnotation(
    annotation: AnnotationStars | AnnotationValue | string,
  ) {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);

    const isAnnotationString = typeof annotation == "string";
    const isAnnotationStars = Object.values(AnnotationStars).includes(
      annotation as AnnotationStars,
    );

    const request = await this.client.requester.request(
      "https://neo.character.ai/annotation/create",
      {
        method: "POST",
        includeAuthorization: true,
        body: Parser.stringify({
          annotation: {
            annotation_type: isAnnotationString
              ? isAnnotationStars
                ? "custom"
                : (annotation as string)
              : "star",
            ...(isAnnotationString
              ? { annotation_raw_content: annotation }
              : undefined),
            annotation_value: isAnnotationString ? 1 : (annotation as number),
          },
          candidate_id: this.candidateId,
          turn_key: {
            chat_id: this.message.turnKey.chatId,
            turn_id: this.message.turnKey.turnId,
          },
        }),
  ...
  async removeAnnotation(annotationId: string) {
    ...
    const request = await this.client.requester.request(
      "https://neo.character.ai/annotation/remove",
      {
        method: "POST",
        includeAuthorization: true,
        body: Parser.stringify({
  ...
  private async internalGetTTSUrl(
    voiceId?: string,
    voiceQuery?: string,
  ): Promise<string> {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);
  ...
  async getTTSUrlWithQuery(voiceQuery?: string) {
    return await this.internalGetTTSUrl(undefined, voiceQuery ?? "");
  }
  async getTTSUrl(voiceId: string) {
    return await this.internalGetTTSUrl(voiceId);
  }
...
export class EditedCandidate extends Candidate {
  ...
  @getterProperty
  public get baseCandidateId() {
    return this.base_candidate_id;
  }
  public set baseCandidateId(value) {
    this.base_candidate_id = value;
  }
  ...
  private editor: any = {};
  @hiddenProperty
  public get editorAuthorId() {
    return { authorId: this.editor.author_id };
  }
}
