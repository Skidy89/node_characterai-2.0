...
export class Conversation extends Specable {
  ...
  protected set characterId(value) {
  ...
  @getterProperty
  public get preferredModelType() {
    return this.preferred_model_type;
  }
  protected set preferredModelType(value) {
    this.preferred_model_type = value;
  }
  ...
  private async fetchMessagesViaQuery(
    pinnedOnly: boolean,
    maxMessages = this.maxMessagesStored,
  ) {
  ...
  async refreshMessages() {
    ...
    for (let i = 0; i < messages.length; i++) this.addMessage(messages[i]);
  ...
  async reset() {
    return await this.deleteMessagesInBulk(
      await this.fetchMessagesViaQuery(false, 999999),
    );
  }
  ...
  async deleteMessagesInBulk(
    input: number | string[] | CAIMessage[],
    ...
    if (typeof input == "number") {
      if (input <= 0 || input >= this.maxMessagesStored)
        throw new Error(
          "Invalid deletion range. The input number must be positive and reach within your messageCache limit.",
        );

      // 200 - 50 for example
      const messageCount = this.messages.length;
      const fetchedMessages = this.messages.slice(
        messageCount - input,
        messageCount,
      );
