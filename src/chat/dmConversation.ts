...
export default class DMConversation extends Conversation {
  ...
  async unarchive(
    refreshMessagesAfter: boolean = true,
  ): Promise<DMConversation | void> {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);
  ...
  async duplicate() {
    ...
    if (!lastMessage)
      throw new Error(
        "You must have atleast one message in the conversation to do this",
      );
  ...
  async regenerateMessage(message: CAIMessage) {
    ...
    const request: any = await this.client.sendDMWebsocketCommandAsync({
      ...
      payload: generateBaseRegeneratingPayload(
        this.characterId,
        message.turnId,
        this.client.myProfile.username as any,
        this.chatId,
      ),
    });
