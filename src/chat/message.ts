...
export class CAIMessage extends Specable {
  ...
  public get turnId(): string {
    return this.turnKey.turnId;
  }
  @getterProperty
  public get chatId() {
    return this.turnKey.chatId;
  }
  ...
  public get lastUpdateDate() {
    return new Date(this.last_update_time);
  }
  ...
  @getterProperty
  public get isHuman() {
    return this.author.is_human ?? false;
  }
  @getterProperty
  ...
  async getAuthorProfile() {
    ...
    if (!isHuman)
      throw Error(
        "Failed to fetch author because this is message was not made by a human.",
      );

    return undefined; // fetchProfileByUsername removed
  }
  async getCharacter() {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);
    ...
    if (isHuman)
      throw Error(
        "Failed to fetch character because this is message was not made by a character.",
      );
  ...
  private addCandidate(
    candidateObject: any,
    addAfterToActualRawCandidates: boolean,
  ) {
    ...
    if (candidate.wasFlagged) Warnings.show("contentFiltered");
  ...
  public get content() {
    return this.primaryCandidate?.content ?? "";
  }
  public get wasFlagged() {
    return this.primaryCandidate?.wasFlagged ?? false;
  }
  ...
  async edit(newContent: string, specificCandidateId?: string) {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);
    const candidateId =
      specificCandidateId ?? this.primaryCandidate.candidateId;
  ...
  private async internalSwitchPrimaryCandidate(
    candidate: "next" | "previous" | string,
  ) {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);
    ...
    const primaryCandidateIndex = Object.keys(candidates).indexOf(
    ...
    if (primaryCandidateIndex != -1) {
      if (candidate == "next")
        candidateId =
          candidateValues[primaryCandidateIndex + 1]?.candidateId ?? "";
    ...
    if (candidateId.trim() == "")
      throw new Error(
        "Cannot find the message, it is invalid or it is out of range.",
      );

    const firstRequest = await this.client.requester.request(
      `https://neo.character.ai/annotations/${this.conversation.chatId}/${this.turnId}/${candidateId}`,
      {
        method: "POST",
  ...
  async switchToPreviousCandidate() {
    return this.internalSwitchPrimaryCandidate("previous");
  }
  async switchToNextCandidate() {
    return this.internalSwitchPrimaryCandidate("next");
  }
  async switchPrimaryCandidateTo(candidateId: string) {
    return await this.internalSwitchPrimaryCandidate(candidateId);
  }

  async regenerate() {
    return this.conversation.regenerateMessage(this);
  }
  ...
  public getMessageBefore() {
    return this.getConversationMessageAfterIndex(-1);
  }
  public getMessageAfter() {
    return this.getConversationMessageAfterIndex(1);
  }
  ...
  async pin() {
    await this.handlePin(true);
  }
  async unpin() {
    await this.handlePin(false);
  }
  // https://neo.character.ai/chat/id/copy
  async copyFromHere(isDM: boolean = true): Promise<DMConversation | any> {
    this.client.checkAndThrow(CheckAndThrow.RequiresAuthentication);

    const request = await this.client.requester.request(
      `https://neo.character.ai/chat/${this.conversation.chatId}/copy`,
      {
        method: "POST",
        contentType: "application/json",
    ...
    return isDM
      ? await this.client.fetchLatestDMConversationWith(newChatId)
      : undefined; // fetchGroupChatConversation removed
  }
  async rewindFromHere() {
    ...
    if (this.conversation.getLastMessage()?.turnId == this.turnId)
      throw new Error(
        "You cannot rewind from the last message in the conversation.",
      );

    return await this.conversation.deleteMessagesInBulk(
      await this.getAllMessagesAfter(),
    );
  }
  async delete() {
  ...
  // TTS
  async getTTSUrlWithQuery(voiceQuery?: string) {
    return await this.primaryCandidate.getTTSUrlWithQuery(voiceQuery);
  }
  async getTTSUrl(voiceId: string) {
    return await this.primaryCandidate.getTTSUrl(voiceId);
  }
