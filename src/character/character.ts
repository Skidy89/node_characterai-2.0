...
export class Character extends Specable {
    ...
    /// features
    async getDMs(turnPreviewCount: number = 2, refreshChats: boolean = false): Promise<any[]> {
    ...
    private async internalDM(createNewConversation: boolean, withGreeting?: boolean, specificChatId?: string): Promise<DMConversation> {
        ...
        var conversation;
        if (specificChatId)
            conversation = undefined; // fetchDMConversation removed
    ...
    async getSimilarCharacters() { return undefined; } // getSimilarCharactersTo removed
