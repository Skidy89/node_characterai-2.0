...
import {
  CAIWebsocket,
  CAIWebsocketConnectionType,
  ICAIWebsocketCommand,
  ICAIWebsocketMessage,
} from "./websocket.js";
...
export class CharacterAI {
  ...
  private async resurrectActiveConversations() {
    const promises = Array.from(this.conversations.values()).map(
      (conversation) => conversation.refreshMessages(),
    );
    await Promise.all(promises);
  }

  private dmChatWebsocket: CAIWebsocket | null = null;
  async sendDMWebsocketAsync(
  ...
  async sendDMWebsocketCommandAsync(
    options: ICAIWebsocketCommand,
    conversation?: Conversation,
  ) {
  ...
  async sendGroupChatWebsocketCommandAsync(
    options: ICAIWebsocketCommand,
    conversation?: Conversation,
  ) {
    const requestId = uuidv4();
  ...
  private async openWebsockets() {
    try {
      ...
      let edgeRollout =
        cookies?.match(/edge_rollout=([^;]+)/)?.[1] ??
        cookies?.match(/deployment-target=([^;]+)/)?.[1] ??
        fallbackEdgeRollout;
      if (!edgeRollout) {
      ...
      this.dmChatWebsocket.on("disconnected", async () => {
        if (this.automaticallyReconnectWebsockets) await this.openWebsockets();
      });
      this.groupChatWebsocket.on("disconnected", async () => {
        if (this.automaticallyReconnectWebsockets) await this.openWebsockets();
      });

      this.dmChatWebsocket.once("connected", () =>
        this.resurrectActiveConversations(),
      );
      this.groupChatWebsocket.once("connected", () =>
        this.resurrectActiveConversations(),
      );
    } catch (error) {
  ...
  async fetchLatestDMConversationWith(characterId: string) {
    ...
    const request = await this.requester.request(
      `https://neo.character.ai/chats/recent/${characterId}`,
      {
        method: "GET",
        includeAuthorization: true,
      },
  ...
  async authenticate(sessionToken: string) {
    ...
    if (sessionToken.startsWith("Token "))
      sessionToken = sessionToken.substring(
        "Token ".length,
        sessionToken.length,
      );
  ...
  checkAndThrow(
    ...
    if (
      argument == CheckAndThrow.RequiresNoAuthentication &&
      this.authenticated
    )
