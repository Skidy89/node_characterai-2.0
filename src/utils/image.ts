...
export class CAIImage extends Specable {
  ...
  public get fullUrl() {
    return this._endpointUrl != ""
      ? `${this.imageEndpoint}${this._endpointUrl}`
      : "";
  }
  ...
  // cannot be set
  @hiddenProperty
  private sharpImage?: Sharp = undefined;
  // if sharp image is loaded
  @getterProperty
  public get isSharpImageLoaded() {
    return this.sharpImage != undefined;
  }
  ...
  private async downloadImageBuffer(
    ...
    if (!this.hasImage)
      console.warn(
        "[node_characterai] No images are loaded or assigned to this cached image. This could either mean the image doesn't have any fallback or it doesn't exist.",
      );
  ...
  public async uploadChanges() {
    ...
    const checkRequest = await this.client.requester.request(
      `${betaImageEndpoint}${uploadURL}`,
      {
        method: "GET",
        includeAuthorization: true,
      },
    ...
    if (checkRequest.status != 200)
      throw new Error(await Parser.parseJSON(checkRequest));
  ...
  public changeToFilePath(path: fs.PathOrFileDescriptor) {
    this.makeSharpImage(fs.readFileSync(path));
  }
  /**
  ...
  async changeToBlobOrFile(blobOrFile: Blob | File) {
    this.makeSharpImage(await blobOrFile.arrayBuffer());
  }
  /**
   ...
   */
  async changeToEndpointUrl(
    endpointUrl: string,
    imageEndpoint: string = neoImageEndpoint,
  ) {
  ...
  constructor(
    client: CharacterAI,
    canUploadChanges: Function | boolean = true,
  ) {
    super();
    this.client = client;
    this.canUploadChanges =
      typeof canUploadChanges == "boolean"
        ? () => canUploadChanges
        : canUploadChanges;
  }
}
