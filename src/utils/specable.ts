...
export function hiddenProperty(target: any, propertyKey: string) {
  ...
  target.constructor[serializableFieldsSymbol].push({
    propertyKey,
    show: false,
  });
}
...
function autoSpec(this: any) {
  ...
  for (const field of allFields) {
    ...
    if (
      !decoratedFields.some((decorated: any) => decorated.propertyKey === field)
    ) {
...
export class Specable {
  [Symbol.for("nodejs.util.inspect.custom")]() {
    return autoSpec.bind(this)();
  }
}
export class EventEmitterSpecable extends EventEmitter {
  [Symbol.for("nodejs.util.inspect.custom")]() {
    return autoSpec.bind(this)();
  }
}
